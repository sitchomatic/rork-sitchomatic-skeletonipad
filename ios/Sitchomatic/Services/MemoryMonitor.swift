...
class MemoryMonitor {
    struct Thresholds {
        ...
        init() {
            ...
            softMB = profile.memoryThresholdSoftMB
            highMB = profile.memoryThresholdHighMB
            criticalMB = profile.memoryThresholdCriticalMB
            emergencyMB = profile.memoryThresholdEmergencyMB
        }
    }
    ...
    var recommendedMaxConcurrency: Int {
        let maxPairs = DeviceCapability.performanceProfile.maxConcurrentPairs
        if lastCheckMB > thresholds.criticalMB || deathSpiralDetected { return max(2, maxPairs / 10) }
        if lastCheckMB > thresholds.highMB || preemptiveThrottleActive { return max(4, maxPairs / 4) }
        if lastCheckMB > thresholds.softMB { return max(6, maxPairs / 2) }
        return maxPairs
    }
